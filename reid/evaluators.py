...
def extract_features_cpu(model, data_loader, print_freq=1, output_feature=None):
    model.eval()
...
class Evaluator(object):
    ...
    def evaluate_using_fea(self, query_features, gallery_features, query, gallery):
        print('ok')
        distmat = pairwise_distance(query_features, gallery_features, query, gallery)

        return evaluate_all(distmat, query=query, gallery=gallery)

    def extract_feas(self, loader, output_feature=None):
        features, labels = extract_features(self.model, loader, 1, output_feature)
        print('Ok')
        return features, labels
