...
class Trainer(BaseTrainer):
    ...
    # def _forward(self, inputs, targets):
    #     outputs = self.model(inputs[:64, :] + 0.5 * inputs[64:, :])
    #     if isinstance(self.criterion, torch.nn.CrossEntropyLoss):
    #         loss = self.criterion(outputs, targets[:64])
    #         prec, = accuracy(outputs.data, targets.data[:64])
    #         prec = prec[0]
    #     elif isinstance(self.criterion, TripletLoss):
    #         loss, prec = self.criterion(outputs[:64], targets[:64])
    #     else:
