...
class Market(Dataset):
    ...
    def __init__(self, dtype, data, img_dir, name, height, width, pose_file):
        ...
        self.posefile = pose_file

        if pose_file:
            with open(self.posefile) as file:
                self.poses = json.load(file)
    ...
    def apply_augmentation(self, example, is_train, pose):
        ...
        X = np.array(pose[0:34:2])
        Y = np.array(pose[1:34:2])
        ...
        perH = np.abs(minY - maxY)
        perW = np.abs(minX - maxX)

        crop_pos = [im_width/2, im_height/2]
        #crop_pos = [(x5+x12)/2, (y5 + y12)/2]
        max_d = np.maximum(perH, perW) + 20
    ...
    def __getitem__(self, ind):
            ...
            pose = self.poses[ind]
            img, fname, pid, cam = self.apply_augmentation(example, self.is_train, pose)
...
def main(args):
    ...
    marketTrain = Market('train', train_source, root+'market/bounding_box_train/','train', args.height, args.width, 'data/pose_train.json')
    galleryds = Market('val', gallery, root+'market/bounding_box_test/','gallery', args.height, args.width, 'data/pose_gallery.json')
    querds = Market('val', query, root+'market/query/', 'query', args.height, args.width, 'data/pose_query.json')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-t', '--target', type=str, default='market',
                        choices=['market', 'duke'])
    # images
    parser.add_argument('-b', '--batch-size', type=int, default=16, help="batch size for source")
